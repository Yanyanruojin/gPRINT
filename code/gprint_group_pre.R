...
folder_name1 = paste0(folder_name,'model/group_level/')
folder_name2 = paste0(folder_name,'result/group_level/')
...
rf <- CreateSeuratObject(counts = rf_del, project = "Baron", min.cells = 0)
...
rf$celltype<-rf_type3$celltype
Idents(rf)<-rf$celltype
rf.markers <- FindAllMarkers(rf, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
######
##marker 基因的排序和输出
...
query <- CreateSeuratObject(counts = query_c, project = "muraro", min.cells = 0)
...
marker_order_gene<-ordergenes[ordergenes$gene_id%in%rf.markers$gene,]
rf_del2<-rf_c[marker_order_gene$gene_id,]
...
rf_c_left<-rf_c2[,rownames(rf_type_left)]
...
path_out_rf= paste0(folder_name4,args[1],'_group_all.csv')
path_out_query= paste0(folder_name3,args[2],'_group_all.csv')
