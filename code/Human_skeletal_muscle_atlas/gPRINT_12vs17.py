...
###  nohup python gPRINT_12vs17.py  reference test 


###########################################################

path='/data/yrj/MTJ/人类骨骼肌图谱/cnn_12vs17/'

path_d= path+sys.argv[1]+'_all.csv'  ##reference

path_out= path+'test_14/out'
...
#sample_size=data_last_1.shape[0]//nclass//2
sample_size=50

if X_1[Y_1==Y_1_name[0]].shape[0] > sample_size :
	X_1_balanced, Y_1_balanced = resample(X_1[Y_1==Y_1_name[0]], Y_1[Y_1==Y_1_name[0]], replace=True, n_samples=sample_size)

X_1_balanced=X_1[Y_1==Y_1_name[0]]
Y_1_balanced=Y_1[Y_1==Y_1_name[0]]
for i in range(1,nclass):
	if X_1[Y_1==Y_1_name[i]].shape[0]> sample_size :
		X_1_balanced = np.concatenate([X_1_balanced,resample(X_1[Y_1==Y_1_name[i]], replace=True, n_samples=sample_size)], axis=0)
		Y_1_balanced = np.concatenate([Y_1_balanced,resample(Y_1[Y_1==Y_1_name[i]], replace=True, n_samples=sample_size)], axis=0)
	
	else:
		X_1_balanced = np.concatenate([X_1_balanced,X_1[Y_1==Y_1_name[i]]], axis=0)
		Y_1_balanced = np.concatenate([Y_1_balanced,Y_1[Y_1==Y_1_name[i]]], axis=0)
...
# 分类编码为数字
encoder = LabelEncoder()
Y_1_encoded = encoder.fit_transform(Y_1_balanced)
Y_1_onehot = np_utils.to_categorical(Y_1_encoded)

X_train=X_1_balanced
Y_train=Y_1_onehot
...
#按照参考数据集合进行筛选基因
path_d_2= path+sys.argv[2]+'_all.csv'  ##reference
